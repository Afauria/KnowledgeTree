...
```java
//定义A、B、C类。C继承B，B继承A。PECS原则基于这样一个事实：“向上转型可以自动转换，向下转型需要强制转换”。
...
> 编译时对A进行了类型擦除，方法变为`setValue(Object)`。而B类擦除之后变为了`setValue(String)`。不满足重写的条件。
>
...
> * 我们的意图是重写，因此编译器对桥方法做了特殊处理，使得源码阶段看起来像是重写。
