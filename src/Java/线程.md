...
interrupt不会终止线程，只是设置一个标志位，可以在线程循环中判断`isInterrupted`，如果线程处于wait阻塞状态，会停止等待，抛出`Interrupted`异常
...
> 3. CAS
> 4. 协程await
> 5. Java Future、ExecutorService、Callable
> 6. Lock+多个Condition，挨个唤醒Condition
