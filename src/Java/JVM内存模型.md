...
>
> 永久代 ≠ 方法区：**方法区是JVM规范，永久代是HotSpot VM对于方法区的具体实现**，其他虚拟机没有永久代的概念。
...
1. Java8之前永久代到底是在堆中还是方法区中？
2. Java 8元空间在本地内存：怎么又多了个本地内存？

网上说法很多不一致，或者很含糊。这里说一下我自己的理解：

1. **堆内存 = 堆 + 永久代**
   1. 在Java8之前方法区使用永久代实现，和堆内存地址连续，只不过JVM从逻辑上把方法区分出来，实际上还是属于堆，都会参与GC，因此可以统称为堆内存。所以有时候会看到网上的图把新生代、老年代、永久代都画到堆中。
   2. 在Java8之后移除了永久代，因此GC回收主要针对新生代和老年代。元空间有自己的回收机制。将堆和元空间区分开来，减少GC复杂度，提升效率。
2. 本地内存：根据上面的解释，这里可以理解为JVM堆之外的内存，也叫**堆外内存**。
3. 直接内存：底层硬件共享的缓冲区，可以直接操作，减少内存间的拷贝。例如FrameBuffer。
...
优势：不需要程序员手动释放，减少程序员维护工作。手动释放可能会产生以下问题
...
**full gc通常和major gc等价**：major gc定义比较模糊，有的时候是指full gc，有的时候是指回收老年代。
